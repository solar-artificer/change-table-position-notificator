...
class WindowManager implements AppModule {
  ...
  async enable({app}: ModuleContext): Promise<void> {
    ...
    globalShortcut.register('F13', () => {
      ...
      this.#mainWindow!.webContents.send('restart-timer');
    })
    ...
    ipcMain.handle('timer-rundown', async () => {
      ...
      if (this.#popupWindow === null || this.#popupWindow === undefined) {
        ...
        if (this.#popup instanceof URL) {
          await this.#popupWindow.loadURL(this.#popup.href);
        } else {
          await this.#popupWindow.loadFile(this.#popup.path);
        }
        ...
        this.#popupWindow.show();
        this.#popupWindow.webContents.send('play-timer-sound');
      }
    });
  }
